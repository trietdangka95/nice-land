...
export function TenantAnalyticsScreen({ slug }: { slug: string }) {
  ...
  return <>
    ...
    {!data ? <div className="mt-8 h-40 animate-pulse bg-white" /> : <><div className="mt-8 grid gap-4 sm:grid-cols-2"><DashboardStat label="Lượt xem" value={data.totals.views.toLocaleString("vi-VN")} detail="Đã chống trùng theo visitor/24h" icon={Eye} tone="blue" /><DashboardStat label="Lead" value={data.totals.leads.toLocaleString("vi-VN")} detail="Yêu cầu theo tin đăng" icon={Users} tone="gold" /></div><section className="mt-7 border border-ink/10 bg-white"><div className="border-b border-ink/10 p-5"><h2 className="font-display text-2xl">Tin hiệu quả nhất</h2></div><div className="divide-y divide-ink/10">{data.topPosts.map((post, index) => <div key={post.id} className="grid grid-cols-[40px_1fr_auto_auto] items-center gap-4 p-5"><span className="font-display text-xl text-ink/35">{index + 1}</span><strong className="text-sm">{post.title}</strong><span className="text-xs text-ink/50">{post.views} lượt xem</span><span className="text-xs font-bold text-moss">{post.leads} lead</span></div>)}</div></section></>}
  </>;
}
