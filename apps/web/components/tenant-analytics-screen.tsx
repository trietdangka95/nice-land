...
export function TenantAnalyticsScreen({ slug }: { slug: string }) {
  ...
  useEffect(() => { void client.getTenantAnalytics().then(setData).catch((e) => toast.error(getErrorMessage(e, "Không thể tải analytics."), "Không thể tải dữ liệu")); }, [client, toast]);
  return <>
