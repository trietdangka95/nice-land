    ...
    <>
      ...
      {loading ? <div className="mt-8 h-36 animate-pulse bg-white" /> : <>
        ...
        <section className="mt-7 border border-ink/10 bg-white">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left"><thead className="bg-[#f8f8f5] text-[10px] uppercase tracking-widest text-ink/40"><tr><th className="px-5 py-4">Website</th><th className="px-5 py-4">Gói</th><th className="px-5 py-4">Sử dụng</th><th className="px-5 py-4">Trạng thái</th><th className="px-5 py-4">Hết hạn</th></tr></thead><tbody className="divide-y divide-ink/10">{sites.slice(0, 8).map((site) => <tr key={site.id}><td className="px-5 py-4"><strong className="block text-sm">{site.name}</strong><span className="text-xs text-ink/40">{site.slug}.datcuatoi.vn</span></td><td className="px-5 py-4 text-sm">{site.plan?.name ?? "Chưa gán"}</td><td className="px-5 py-4 text-xs text-ink/55">{site.usage.posts} tin · {site.usage.images} ảnh</td><td className="px-5 py-4"><StatusPill tone={site.isActive ? "green" : "red"}>{site.isActive ? site.subscriptionStatus : "Tạm ngưng"}</StatusPill></td><td className="px-5 py-4 text-xs text-ink/45">{site.subscriptionEnd ? new Date(site.subscriptionEnd).toLocaleDateString("vi-VN") : "—"}</td></tr>)}</tbody></table></div>
        </section>
      </>}
    </>
  );
